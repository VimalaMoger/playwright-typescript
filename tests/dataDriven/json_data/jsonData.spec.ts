...
let page: Page;
// approach I
    //for (const index of loginData || []) {  // Fallback to empty array
// approach II
// check if loginData is an array
if (Array.isArray(loginData)) {    
    for (const [email, password, firstName] of loginData.map((item: {email: string, password: string, firstName: string}) => [item.email, item.password, item.firstName])) {
        ...
        test.describe(`Grouping multiple tests ${firstName}`, {tag: ['@grouping','@regression']}, async () => {
            ...
            test('Login', async () => {
                ...
                await page.locator('#fName').fill(firstName);
                // grab first name 
                const enteredFirstName : string =  await page.locator('#fName').inputValue();
